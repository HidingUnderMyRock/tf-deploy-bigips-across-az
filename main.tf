...
resource "aws_network_interface" "mgmt-enis" {
    ...
    tags = {
        Name                    = "lipowsky-tf-bigip-${count.index+1}-eth0"
    }
}
...
resource "aws_network_interface" "external-enis" {
    ...
    tags = {
        Name                    = "lipowsky-tf-bigip-${count.index+1}-eth1"
    }
}
...
resource "aws_network_interface" "internal-enis" {
    ...
    tags = {
        Name                    = "lipowsky-tf-bigip-${count.index+1}-eth2"
    }
}
...
resource "aws_instance" "f5_bigip" {
    ...
    tags = {
        Name                    = "lipowsky-tf-bigip-${count.index}"
    }
}